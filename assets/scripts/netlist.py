...
# ==================
import sys
sys.path.append('C:/Users/USER/AndroidStudioProjects/circuts2/assets/scripts/scipy')
...
class Network:
    ...
    def __init__(self, raw_text):
        ...
        (self.names,
         ...
         self.analysis,
         self.plot_cmd,
         self.tf_cmd) = self.read_netlist(raw_text)


    def read_netlist(self, raw_text):
        """
    ...
    def print(self, variable='all', polar=False, message=False):
        ...
        voltage_fmt = 'v({}) = {} V\n'
        voltage_fmt_polar = 'v({}) = {} V < {}°\n'
        current_fmt = 'i({}) = {} A\n'
        current_fmt_polar = 'i({}) = {} A < {}°\n'
        power_fmt = 'p({}) = {} {}\n'
        power_fmt_polar = 'p({}) = {} {} < {}°\n'
        ...
        if variable.lower() == 'voltage':
            ...
            for k, index in enumerate(self.isort):
                if polar:
                    if k == 0:  # resistors
                        for h in index:
                            msg += voltage_fmt_polar.format(self.names[h], list(np.abs(self.vb[h])) if isinstance(self.ib[h], np.ndarray) else np.abs(self.vb[h]), np.angle(self.vb[h], deg=True))
                            msg += '----------------------------------------------\n'
                    ...
                    if k == 0:  # resistors
                        for h in index:
                            msg += voltage_fmt.format(self.names[h], list(self.vb[h]) if isinstance(self.ib[h], np.ndarray) else self.vb[h])
                            msg += '----------------------------------------------\n'
                    elif k == 1:  # voltage sources
                        for h in index:
                            msg += voltage_fmt.format(self.names[h], list(self.vb[h]) if isinstance(self.ib[h], np.ndarray) else self.vb[h])
            ...
            for k, index in enumerate(self.isort):
                if polar:
                    ...
                    if k == 0:  # resistors
                        for h in index:
                            msg += current_fmt.format(self.names[h], list(self.ib[h]) if isinstance(self.ib[h], np.ndarray) else self.ib[h])
                            msg += '----------------------------------------------\n'
                    elif k == 1:  # inductors
                        for h in index:
                            msg += current_fmt.format(self.names[h], list(self.ib[h]) if isinstance(self.ib[h], np.ndarray) else self.ib[h])
